...
export default function Index() {
  ...
  const loadNoteContents = async () => {
    try {
      const contents = await noteContentApi.getAllNoteContent('default'); // TODO: Get actual user ID
  ...
  const loadReviewNotes = async () => {
    try {
      const dueNotes = await notesApi.getDueForReview();
      ...
      <div className="flex-1 relative z-10">
        ...
        <div className="mx-auto max-w-7xl px-6 py-8">
          ...
          <div className="w-full">
            <Card className="overflow-hidden border-2 shadow-lg bg-card">
                  ...
                  {noteContents.map(noteContent => {
                    ...
                    return (
                      <NoteCard
                        ...
                        difficultVocabularies={noteContent.difficultVocabularies}
                        onVocabularyCheck={async (vocabulary) => {
                          try {
                            // Save vocabulary as a flashcard
                            await flashcardsApi.createFlashcard({
                              front: vocabulary,
                              back: null, // Can be filled in later
                              sourceUrl: noteContent.url,
                              tag: 'vocabulary',
                              userId: 'default'
                            });
                            console.log('Flashcard created for:', vocabulary);
                          } catch (error) {
                            console.error('Error saving vocabulary as flashcard:', error);
                            setError('Failed to save vocabulary as flashcard');
                          }
                        }}
                      />
                    );
